...
const EditQASpecialistForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchQAData = async () => {
      try {
        const response = await adminService.getQA(id as string);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await adminService.updateQA(id as string, formDataToSend);
      if (response.status === 200) {
  ...
  return (
    <div className="bg-gray-100 min-h-full p-8 overflow-y-auto">
