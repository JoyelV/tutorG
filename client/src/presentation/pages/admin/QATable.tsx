...
const QATeamTable: React.FC = () => {
  ...
  const fetchQAData = () => {
    ...
    adminService
      ...
      .then((response) => {
        ...
        setQaData(data.filter((u: any) => u.role === 'QA Specialist') || data);
        setLoading(false);
      })
      .catch((error) => {
  ...
  return (
    <div className="bg-gray-100 min-h-full p-6 overflow-y-auto">
      {/* Toast Container */}
      <ToastContainer />
      ...
      <div className="flex space-x-4 mb-4">
        {["QA Specialist", "Requests"].map((tab) => (
          <button
            ...
            className={`text-blue-500 ${activeTab === tab ? "border-b-2 border-blue-500" : ""
              }`}
          >
            {tab}
          </button>
                    ...
                    filteredData.map((item: any, index: number) => (
                      ...
                      >
                        ...
                        <td className="border border-gray-300 px-4 py-2 flex gap-2">
                          <button
                            className={`px-4 py-1 rounded-md ${item.isDeleted
