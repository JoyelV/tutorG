    ...
    <div className="mx-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 justify-center">
      {instructors.map((instructor, index) => (
        <div key={index} className="bg-white rounded-xl shadow-md max-w-xs w-full mx-auto transform transition duration-500 hover:scale-105 hover:shadow-xl">
          {/* Instructor Image */}
          <div className="h-60 bg-cover bg-center rounded-t-xl" style={{ backgroundImage: `url(${instructor.image})` }}></div>
          <div className="p-3">
            ...
            ★  {instructor.rating}
              ...
              <div><span className="text-gray-500 font-semibold">{instructor.students} students</span></div>
            </div>
          </div>
        </div>
      ))}
